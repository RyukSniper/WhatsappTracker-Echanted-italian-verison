document.addEventListener('DOMContentLoaded', function (event) {
    ...
    window.outputToLog = function (message) {
        ...
        messageCounter++;
        if (messages.length > MAX_LOG_LENGTH) messages.shift();
        let log = '';
...
window.onmessage = function (e) {
    ...
    case 'wa_stream_start':
        ...
        break;
    case 'wa_stream_end':
        if (state == State.RECORDING) state = State.LOGGED_IN;
        break;
    case 'wa_logged_in':
        if (state == State.LOGGED_OUT) state = State.LOGGED_IN;
        break;
    case 'wa_logged_out':
    ...
    if (oldState != state) {
        ...
        case State.LOGGED_OUT:
            text = 'Fai il Login su Whatsapp per registrare:';
            break;
        case State.LOGGED_IN:
            text = 'Login effettuato ma non sto registrando';
            break;
